...
interface KiroApi {
  ...
  // 从 Kiro 本地配置导入凭证
  // Removed loadKiroCredentials. User must add account manually.
  ...
  // ============ Authentication API ============
  auth: {
    login: (email: string, password: string) => Promise<{ success: boolean; error?: string; user?: { email: string; role: 'admin' | 'user'; createdAt: Date; lastLogin: Date } }>
    register: (email: string, password: string) => Promise<{ success: boolean; error?: string }>
    logout: (email: string) => Promise<{ success: boolean; error?: string }>
    getAllUsers: (requestingUserEmail: string) => Promise<{ success: boolean; users?: Array<{ _id: any; email: string; role: 'admin' | 'user'; createdAt: Date; lastLogin?: Date }>; error?: string }>
  }
