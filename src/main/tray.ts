...
function getTrayIconDir(): string {
  // 开发环境和生产环境路径不同
  if (app.isPackaged) {
    // asarUnpack will unpack resources to app.asar.unpacked directory
    return join(process.resourcesPath, 'app.asar.unpacked', 'resources', 'tray-icons')
  }
  return join(__dirname, '../../resources/tray-icons')
}
...
const ICON_FILE_MAP: Record<string, string> = {
  ...
  'app': 'icon.png',
  // Status icons
  'status-running': 'status-running.png',
  'status-stopped': 'status-stopped.png',
  // Menu icons
  'mail': 'account.png',
  'refresh': 'refresh.png',
...
function buildTrayMenu(): Menu {
  ...
  // 应用标题
  menuTemplate.push({
    label: `Mira AI v${app.getVersion()}`,
...
export function createTray(cbs: TrayCallbacks): Tray | null {
  ...
  try {
    ...
    tray = new Tray(icon)
    tray.setToolTip(currentLanguage === 'en' ? 'Mira AI' : 'Mira AI')
    tray.setContextMenu(buildTrayMenu())
