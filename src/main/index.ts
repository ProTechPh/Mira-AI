...
import { fetchKiroModels, fetchSubscriptionToken, fetchAvailableSubscriptions, setUseKProxyForApiInProxy } from './proxy/miraApi'
...
let isAuthenticated = false // 标记用户是否已登录
...
function initTray(): void {
  ...
  // 设置初始提示
  setTrayTooltip(`Mira AI v${app.getVersion()}`)
}

function createWindow(): void {
  // Create the browser window.
  mainWindow = new BrowserWindow({
    title: `Mira AI v${app.getVersion()}`,
  ...
  mainWindow.on('ready-to-show', () => {
    // Title will be set by renderer based on language preference
    mainWindow?.show()
...
function registerProtocol(): void {
  ...
  const protocolEntrypoint = process.argv.length >= 2 ? resolve(process.argv[1]) : null

  // 开发模式/非打包模式：需要显式传入入口脚本路径
  if (!app.isPackaged && protocolEntrypoint) {
    app.setAsDefaultProtocolClient(PROTOCOL_PREFIX, process.execPath, [protocolEntrypoint])
  } else {
...
function unregisterProtocol(): void {
  ...
  if (!app.isPackaged && protocolEntrypoint) {
    app.removeAsDefaultProtocolClient(PROTOCOL_PREFIX, process.execPath, [protocolEntrypoint])
  } else {
...
app.whenReady().then(async () => {
  ...
  ipcMain.on('update-tray-language', (_event, language: 'en' | 'zh') => {
    ...
    const title = `Mira AI v${app.getVersion()}`
    mainWindow?.setTitle(title)
  })

  // IPC: 设置认证状态
  ipcMain.on('set-authenticated', (_event, authenticated: boolean) => {
  ...
  const GITHUB_REPO = 'ProTechPh/Mira-Release'
  ...
  // IPC: 加载账号数据 (Admin only - loads all accounts from MongoDB)
  // IPC: 加载账号数据
  ...
  ipcMain.handle('import-from-sso-token', async (_event, bearerToken: string, region: string = 'us-east-1') => {
    ...
    try {
      ...
      const subscriptionTitle = usageData?.subscriptionInfo?.subscriptionTitle || 'MIRA AI'
  ...
  // IPC: 从 Kiro 本地配置导入凭证
  // Removed load-kiro-credentials handler. User must add account manually.
...
`
  ...
  ipcMain.handle('proxy-add-api-key', async (_event, apiKey: { name: string; key?: string; format?: 'sk' | 'simple' | 'token'; creditsLimit?: number }) => {
    try {
      ...
      const newApiKey: import('./proxy/mTypes').ApiKey = {
  ...
  // IPC: 更新 API Key
  ipcMain.handle('proxy-update-api-key', (_event, id: string, updates: Partial<import('./proxy/mTypes').ApiKey>) => {
    try {
      const server = initProxyServer()
      const config = server.getConfig()
      const apiKeys = config.apiKeys || []
