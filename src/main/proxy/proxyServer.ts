...
import { callKiroApiStream, callKiroApi, fetchKiroModels, type KiroModel } from './miraApi'
import { proxyLogger } from './logger'
import { getKProxyService, generateDeviceId } from '../mproxy'
import {
  openaiToKiro,
  claudeToKiro,
...
export class ProxyServer {
  ...
  private validateApiKey(req: http.IncomingMessage): { valid: boolean; apiKey?: import('./mTypes').ApiKey; reason?: string } {
  ...
  private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    if (path !== '/health' && path !== '/') {
      ...
      ;(req as unknown as { matchedApiKey?: import('./mTypes').ApiKey }).matchedApiKey = authResult.apiKey
    }
  ...
  private async handleModels(res: http.ServerResponse): Promise<void> {
    ...
    const kiroOfficialModels = [
      ...
      { id: 'claude-3.7-sonnet', object: 'model', created: now, owned_by: 'kiro-api', description: 'Claude 3.7 Sonnet model' },
      { id: 'claude-haiku-4.5', object: 'model', created: now, owned_by: 'kiro-api', description: 'The latest Claude Haiku model' },
      { id: 'claude-sonnet-4', object: 'model', created: now, owned_by: 'kiro-api', description: 'Claude Sonnet 4 model' },
      { id: 'claude-sonnet-4.5', object: 'model', created: now, owned_by: 'kiro-api', description: 'The latest Claude Sonnet model' },
      { id: 'deepseek-3.2', object: 'model', created: now, owned_by: 'kiro-api', description: 'DeepSeek 3.2 model' },
      { id: 'minimax-m2.1', object: 'model', created: now, owned_by: 'kiro-api', description: 'MiniMax M2.1 model' },
      { id: 'qwen3-coder-next', object: 'model', created: now, owned_by: 'kiro-api', description: 'Qwen3 Coder Next model' }
    ]
  ...
  private async handleOpenAIChat(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    const matchedApiKey = (req as unknown as { matchedApiKey?: import('./mTypes').ApiKey }).matchedApiKey
  ...
  private async handleOpenAIStream(
    ...
    matchedApiKey?: import('./mTypes').ApiKey
  ): Promise<void> {
    if (!headersSent) {
      res.writeHead(200, {
