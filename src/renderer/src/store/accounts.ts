...
export const useAccountsStore = create<AccountsStore>()((set, get) => ({
  ...
  refreshAccountToken: async (id) => {
    ...
    try {
      // 通过主进程调用 Mira AI API 刷新 Token（避免 CORS）
  ...
  checkAccountStatus: async (id) => {
    ...
    try {
      // 通过主进程调用 Mira AI API 获取状态（避免 CORS）
  ...
  loadFromStorage: async () => {
    ...
    try {
        ...
        try {
          ...
          if (localResult.success && localResult.data?.refreshToken) {
            ...
            if (foundAccountId) {
              ...
              // If not found, skip auto-import (manual add only)
              console.log('[Store] Local account not found in app; skipping import.')
            }
          }
        } catch (e) {
      ...
      console.warn('[Store] Failed to load accounts:', e)
      set({ isLoading: false })
    }
  },
