...
export function AddAccountDialog({ isOpen, onClose }: AddAccountDialogProps): React.ReactNode {
  ...
  const socialExpectedStateRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.onSocialAuthCallback(async (data) => {
      ...
      if (data.code && data.state) {
        // Ignore stale callbacks from older login attempts.
        if (socialExpectedStateRef.current && data.state !== socialExpectedStateRef.current) {
          console.log('[AddAccountDialog] Ignoring stale social auth callback state')
          return
        }
        ...
        try {
          ...
          if (result.success) {
            ...
            // Duplicate callbacks can arrive after success/cancel. Keep waiting instead of failing hard.
            if (result.error?.includes('没有进行中的社交登录')) {
              console.log('[AddAccountDialog] No active social login in main process, ignoring duplicate callback')
              return
            }
          ...
          socialExpectedStateRef.current = null
          setIsLoggingIn(false)
        }
      }
    })
  ...
  const handleStartSocialLogin = async (socialProvider: 'Google' | 'Github') => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        socialExpectedStateRef.current = result.state || null
      }
      // 成功后等待回调
    } catch (e) {
      setError(e instanceof Error ? e.message : '启动登录失败')
  ...
  const handleImportFromLocal = async () => {
    // Auto-import from local config is no longer supported.
    setError('Automatic import from local config is no longer supported. Please add your account manually.')
  }
