...
export function McpServerEditor({ serverName, server, onClose, onSaved }: McpServerEditorProps) {
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      setError(t('mcpServerEditor.pleaseEnterServerName'))
      return
    }
    if (!command.trim()) {
      setError(t('mcpServerEditor.pleaseEnterCommand'))
      return
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('mcpServerEditor.saveFailed'))
      }
    } catch (err) {
      setError(t('mcpServerEditor.saveFailed'))
      console.error(err)
    } finally {
      setSaving(false)
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-background rounded-lg shadow-xl w-[90vw] max-w-lg max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <h2 className="font-semibold">{isEdit ? t('mcpServerEditor.editMcpServer') : t('mcpServerEditor.addMcpServer')}</h2>
        ...
        <div className="flex-1 p-4 space-y-4 overflow-auto">
          {/* 服务器名称 */}
          <div>
            <label className="block text-sm font-medium mb-1">{t('mcpServerEditor.serverName')}</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={t('mcpServerEditor.serverNamePlaceholder')}
          ...
          {/* 命令 */}
          <div>
            <label className="block text-sm font-medium mb-1">{t('mcpServerEditor.command')}</label>
            <input
              type="text"
              value={command}
              onChange={(e) => setCommand(e.target.value)}
              placeholder={t('mcpServerEditor.commandPlaceholder')}
              className="w-full px-3 py-2 rounded-md border bg-background text-sm"
            />
          </div>
          ...
          {/* 参数 */}
          <div>
            <label className="block text-sm font-medium mb-1">{t('mcpServerEditor.arguments')}</label>
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t('mcpServerEditor.addArgument')}
          ...
          {/* 环境变量 */}
          <div>
            <label className="block text-sm font-medium mb-1">{t('mcpServerEditor.environmentVariables')}</label>
            <div className="space-y-2">
              {envVars.map((env, index) => (
                <div key={index} className="flex gap-2">
                  <input
                    ...
                    placeholder={t('mcpServerEditor.key')}
                  ...
                  <input
                    ...
                    placeholder={t('mcpServerEditor.value')}
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t">
          <Button variant="outline" onClick={onClose}>{t('mcpServerEditor.cancel')}</Button>
