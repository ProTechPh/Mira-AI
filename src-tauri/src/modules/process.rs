...
pub fn close_antigravity(timeout_secs: u64) -> Result<(), String> {
    ...
    crate::modules::logger::log_info("in progress: close Antigravity...");

    let pids = collect_antigravity_main_pids();
    if pids.is_empty() {
 crate::modules::logger::log_info("Antigravity ，close");
        return Ok(());
    }

 crate::modules::logger::log_info(&format!("close {} Antigravity ...", pids.len()));
 ...
 crate::modules::logger::log_info("Antigravity succeededclose");
    Ok(())
}
...
pub fn start_antigravity_with_args(
    ...
    crate::modules::logger::log_info("in progress: start Antigravity...");
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Some(path) = launch_path {
            ...
            match spawn_detached_unix(&mut cmd) {
                Ok(child) => {
 crate::modules::logger::log_info("Antigravity start");
                    return Ok(child.id());
                }
                Err(e) => {
...
pub fn start_codex_with_args(codex_home: &str, extra_args: &[String]) -> Result<u32, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        if let Some(path) = launch_path {
            ...
            match spawn_detached_unix(&mut cmd) {
                Ok(child) => {
 crate::modules::logger::log_info("Codex start");
                    return Ok(child.id());
                }
                Err(e) => {
                    if codex_home.trim().is_empty() {
...
pub fn close_codex(timeout_secs: u64) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        crate::modules::logger::log_info("in progress: close Codex...");
...
pub fn close_codex_instances(codex_homes: &[String], timeout_secs: u64) -> Result<(), String> {
 ...
 crate::modules::logger::log_info("close Codex ");
            return Ok(());
        }
        ...
        pids.dedup();
        if pids.is_empty() {
 crate::modules::logger::log_info(" Codex ，close");
            return Ok(());
        }
...
pub fn close_opencode(timeout_secs: u64) -> Result<(), String> {
    ...
    crate::modules::logger::log_info("in progress: close OpenCode...");
    let pids = get_opencode_pids();
    if pids.is_empty() {
 crate::modules::logger::log_info("OpenCode ，close");
        return Ok(());
    }

 crate::modules::logger::log_info(&format!("close {} OpenCode ...", pids.len()));
    let _ = close_pids(&pids, timeout_secs);
 ...
 crate::modules::logger::log_info("OpenCode succeededclose");
    Ok(())
}

/// 启动 OpenCode（桌面端）
pub fn start_opencode_with_path(custom_path: Option<&str>) -> Result<(), String> {
    crate::modules::logger::log_info("in progress: start OpenCode...");
 ...
 crate::modules::logger::log_info(&format!("OpenCode start: {}", target));
        return Ok(());
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        for candidate in candidates {
            ...
            if Command::new(&candidate)
                ...
                .is_ok()
            {
 crate::modules::logger::log_info(&format!("OpenCode start: {}", candidate));
                return Ok(());
            }
        }
...
pub fn start_vscode_with_args_with_new_window(
 ...
 crate::modules::logger::log_info("VS Code start");
        return Ok(child.id());
    }
...
pub fn start_vscode_default_with_args_with_new_window(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let child =
            spawn_detached_unix(&mut cmd).map_err(|e| format!("启动 VS Code 失败: {}", e))?;
 crate::modules::logger::log_info("VS Code default instancestart");
        return Ok(child.id());
    }
