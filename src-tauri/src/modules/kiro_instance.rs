...
pub fn get_default_instances_root_dir() -> Result<PathBuf, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        return Ok(home.join(".antigravity_mira/instances/kiro"));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return Ok(PathBuf::from(appdata).join(".antigravity_mira\\instances\\kiro"));
    }
...
pub fn detect_and_save_kiro_launch_path(force: bool) -> Option<String> {
 ...
 modules::logger::log_warn(&format!("save Kiro startpathfailed（）: {}", err));
        }
    }
    Some(normalized)
}
