...
pub fn write_sync_setting(key: &str, value: &str) {
    ...
    match key {
        ...
        _ => {
 crate::modules::logger::log_warn(&format!("[SyncSettings] : {}", key));
            return;
        }
    }

    if let Err(e) = save_sync_settings(&settings) {
 crate::modules::logger::log_error(&format!("[SyncSettings] writefailed: {}", e));
    } else {
...
pub fn clear_sync_setting(key: &str) {
    ...
    if had_value {
        if let Err(e) = save_sync_settings(&settings) {
 crate::modules::logger::log_error(&format!("[SyncSettings] failed: {}", e));
        } else {
 crate::modules::logger::log_info(&format!("[SyncSettings] : {}", key));
        }
    }
}
