...
const PROXY_DIR: &str = "kiro_proxy";
...
pub fn proxy_data_dir() -> Result<PathBuf, String> {
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir).map_err(|e| format!("创建 Kiro Proxy 目录失败: {}", e))?;
    }
    Ok(dir)
}
...
pub fn load_model_cache() -> Result<Option<ModelCacheState>, String> {
    ...
    let content =
        fs::read_to_string(&path).map_err(|e| format!("读取 Proxy 模型缓存失败: {}", e))?;
...
pub fn save_model_cache(cache: &ModelCacheState) -> Result<(), String> {
    let path = models_cache_path()?;
    let content =
        serde_json::to_string(cache).map_err(|e| format!("序列化 Proxy 模型缓存失败: {}", e))?;
