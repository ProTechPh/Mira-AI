...
pub fn get_default_instances_root_dir() -> Result<PathBuf, String> {
    ...
    {
        let home = dirs::home_dir().ok_or("无法获取用户主目录")?;
        return Ok(home.join(".antigravity_mirances/windsurf"));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        let appdata =
            std::env::var("APPDATA").map_err(|_| "无法获取 APPDATA 环境变量".to_string())?;
        return Ok(PathBuf::from(appdata).join(".antigravity_mira\\instances\\windsurf"));
    }

    #[cfg(target_os = "linux")]
    {
        let home = dirs::home_dir().ok_or("无法获取用户主目录")?;
        return Ok(home.join(".antigravity_mira/instances/windsurf"));
    }
...
pub fn detect_and_save_windsurf_launch_path(force: bool) -> Option<String> {
 ...
 modules::logger::log_warn(&format!("save Windsurf startpathfailed（）: {}", err));
        }
    }
    Some(normalized)
}
