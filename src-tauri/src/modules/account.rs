...
pub fn list_accounts() -> Result<Vec<Account>, String> {
 modules::logger::log_info("start account...");
    ...
    for summary in &index.accounts {
        match load_account(&summary.id) {
            ...
            Err(e) => {
 modules::logger::log_error(&format!("accountfailed: {}", e));
            }
        }
    }
...
pub fn upsert_account(
    ...
    if let Some(account_id) = existing_account_id {
        match load_account(&account_id) {
            ...
            Err(e) => {
 modules::logger::log_warn(&format!("account，in progress: : {}", e));
                let mut account = Account::new(account_id.clone(), email.clone(), token);
                account.name = name.clone();
                let fingerprint = crate::modules::fingerprint::generate_fingerprint(email.clone())?;
                account.fingerprint_id = Some(fingerprint.id.clone());
...
fn save_current_account_file(email: &str) -> Result<(), String> {
 ...
 modules::logger::log_info("saveaccount");
    Ok(())
}
...
pub fn send_quota_alert_native_notification(payload: &QuotaAlertPayload) {
    ...
    if let Err(e) = app_handle
        ...
        .show()
    {
 modules::logger::log_warn(&format!("[QuotaAlert] failed: {}", e));
    }
}
...
pub async fn run_auto_switch_if_needed() -> Result<Option<Account>, String> {
 ...
 modules::logger::log_info("[AutoSwitch] auto switch，");
        return Ok(None);
    }
...
pub async fn switch_account_internal(account_id: &str) -> Result<Account, String> {
    modules::logger::log_info("[Switch] start switch account");

    // 1. 加载并验证账号存在
    let mut account = prepare_account_for_injection(account_id).await?;
 modules::logger::log_info("[Switch] in progress: account");
 ...
 modules::logger::log_info("[Switch] write");
 ...
 modules::logger::log_warn(&format!("[Switch] default instanceaccountfailed: {}", e));
    }
 ...
 modules::logger::log_info(&format!("[Switch] default instance PID={}，close", pid));
    ...
    modules::logger::log_info("[Switch] in progress: start Antigravity default instance...");
    match modules::process::start_antigravity() {
        Ok(pid) => {
            let _ = modules::instance::update_default_pid(Some(pid));
        }
        Err(e) => {
            modules::logger::log_warn(&format!("[Switch] Antigravity startfailed: {}", e));
            // 不中断流程，允许用户手动启动
        }
    }

 modules::logger::log_info("[Switch] accountcompleted");
    Ok(account)
}
...
pub async fn prepare_account_for_injection(account_id: &str) -> Result<Account, String> {
 ...
 modules::logger::log_info("[Account] Token refresh");
