        ...
        .setup(|app| {
            ...
            // 启动时同步：读取共享配置文件，与本地配置比较时间戳后合并
            {
                ...
                if let Some(merged_language) = modules::sync_settings::merge_setting_on_startup(
                    ...
                    info!(
                        "[SyncSettings] Merged language setting at startup: {} -> {}",
                    ...
                    if let Err(e) = modules::config::save_user_config(&new_config) {
                        logger::log_error(&format!(
                            "[SyncSettings] Failed to save merged config: {}",
                            e
                        ));
                    }
                }
            ...
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                match config.close_behavior {
                    CloseWindowBehavior::Minimize => {
                        ...
                        api.prevent_close();
                        let _ = window.hide();
                        info!("[Window] Window minimized to tray");
                    }
                    CloseWindowBehavior::Quit => {
                        // 直接退出，不阻止关闭
                        info!("[Window] User selected to quit app");
                    }
                    CloseWindowBehavior::Ask => {
                        // 需要询问用户，阻止关闭并发送事件到前端
                        api.prevent_close();
                        let _ = window.emit("window:close_requested", ());
                        info!("[Window] Waiting for user close action selection");
                    }
                }
            }
        })
