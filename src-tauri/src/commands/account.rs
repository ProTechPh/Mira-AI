...
pub async fn refresh_all_quotas(
    ...
    if result.is_ok() {
        ...
        match modules::account::run_auto_switch_if_needed().await {
            Ok(Some(account)) => {
                modules::logger::log_info(&format!("[AutoSwitch] auto switchcompleted: {}", account.email));
                switched = true;
            }
            Ok(None) => {}
            Err(e) => {
 modules::logger::log_warn(&format!("[AutoSwitch] auto switchfailed: {}", e));
            }
        }
        if !switched {
            if let Err(e) = modules::account::run_quota_alert_if_needed() {
                modules::logger::log_warn(&format!("[QuotaAlert] alert checkfailed: {}", e));
            }
        }
        let _ = crate::modules::tray::update_tray_menu(&app);
    }
...
pub async fn refresh_current_quota(app: tauri::AppHandle) -> Result<(), String> {
    ...
    match modules::account::run_auto_switch_if_needed().await {
        ...
        Ok(None) => {}
        Err(e) => {
 modules::logger::log_warn(&format!("[AutoSwitch] accountrefreshauto switchfailed: {}", e));
        }
    }

    if !switched {
        if let Err(e) = modules::account::run_quota_alert_if_needed() {
 modules::logger::log_warn(&format!("[QuotaAlert] accountrefreshalert checkfailed: {}", e));
        }
    }
...
pub async fn switch_account(app: AppHandle, account_id: String) -> Result<models::Account, String> {
    modules::logger::log_info(&format!("start switch account: {}", account_id));
 ...
 modules::logger::log_info(&format!("Token refresh: {}", account.email));
 ...
 modules::logger::log_warn(&format!(" Antigravity default instanceaccountfailed: {}", e));
    } else {
        modules::logger::log_info(&format!(
 ...
 modules::logger::log_info(&format!("default instance PID: {}，close", pid));
    ...
    modules::logger::log_info("in progress: start Antigravity default instance...");
    match modules::process::start_antigravity() {
        Ok(pid) => {
            if let Err(e) = modules::instance::update_default_pid(Some(pid)) {
 modules::logger::log_warn(&format!("default instance PID failed: {}", e));
            }
        }
        Err(e) => {
            modules::logger::log_warn(&format!("Antigravity startfailed: {}", e));
 ...
 modules::logger::log_info(&format!("accountcompleted: {}", account.email));
...
pub async fn sync_current_from_client() -> Result<Option<String>, String> {
 ...
 modules::logger::log_info("[SyncClient] localaccount Tools ");
    Ok(None)
}
