...
pub fn save_general_config(
 ...
 modules::logger::log_warn(&format!("[Tray] language changerefreshtrayfailed: {}", err));
        }
    }
...
pub fn handle_window_close(
    ...
    if remember {
        ...
        config::save_user_config(&new_config)?;
 modules::logger::log_info(&format!("[Window] saveclose: {}", action));
    }
    ...
    match action.as_str() {
        "minimize" => {
            let _ = window.hide();
 modules::logger::log_info("[Window] tray");
        }
        "quit" => {
