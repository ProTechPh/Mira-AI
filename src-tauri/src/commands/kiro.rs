...
use crate::models::kiro::{KiroAccount, KiroOAuthStartOptions, KiroOAuthStartResponse};
    ...
    match kiro_account::refresh_account_token(&account_id).await {
        Ok(account) => {
            if let Err(e) = kiro_account::run_quota_alert_if_needed() {
                logger::log_warn(&format!("[QuotaAlert][Kiro] alert checkfailed: {}", e));
            }
...
pub async fn refresh_all_kiro_tokens(app: AppHandle) -> Result<i32, String> {
    let started_at = Instant::now();
 logger::log_info("[Kiro Command] refreshstart ");
    ...
    if success_count > 0 {
        if let Err(e) = kiro_account::run_quota_alert_if_needed() {
 logger::log_warn(&format!("[QuotaAlert][Kiro] refreshalert checkfailed: {}", e));
        }
    }
...
pub async fn kiro_oauth_login_start(
    options: Option<KiroOAuthStartOptions>,
) -> Result<KiroOAuthStartResponse, String> {
    logger::log_info("Kiro OAuth start command triggered");
...
pub async fn inject_kiro_to_vscode(app: AppHandle, account_id: String) -> Result<String, String> {
 ...
 logger::log_warn(&format!(" Kiro default instanceaccountfailed: {}", err));
    }
    ...
    .await
    {
        ...
        Err(err) => {
            if err.starts_with("APP_PATH_NOT_FOUND:") || err.contains("启动 Kiro 失败") {
                logger::log_warn(&format!("Kiro default instancestartfailed: {}", err));
