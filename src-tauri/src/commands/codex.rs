...
pub async fn switch_codex_account(
 ...
 logger::log_warn(&format!(" Codex default instanceaccountfailed: {}", e));
    } else {
    ...
    match opencode_auth::replace_openai_entry_from_codex(&account) {
        ...
        Err(e) => {
 logger::log_warn(&format!("OpenCode auth.json : {}", e));
        }
    }
        ...
        if opencode_updated {
            if process::is_opencode_running() {
                if let Err(e) = process::close_opencode(20) {
                    logger::log_warn(&format!("OpenCode closefailed: {}", e));
                }
            } else {
 logger::log_info("OpenCode ，start");
            }
            if let Err(e) = process::start_opencode_with_path(Some(&user_config.opencode_app_path))
            {
                logger::log_warn(&format!("OpenCode startfailed: {}", e));
            }
        } else {
 logger::log_info("OpenCode auth.json，start/");
        }
    } else {
 logger::log_info("close OpenCode ");
    }
 ...
 logger::log_info(" Codex in progress: ，default instance PID ");
            }
                ...
                .await
            {
                Ok(_) => {}
                Err(e) => {
                    logger::log_warn(&format!("Codex startfailed: {}", e));
 ...
 logger::log_info("close Codex start Codex App");
        }
    }
...
pub async fn refresh_codex_quota(app: AppHandle, account_id: String) -> Result<CodexQuota, String> {
    ...
    if result.is_ok() {
        if let Err(e) = codex_account::run_quota_alert_if_needed() {
            logger::log_warn(&format!("[QuotaAlert][Codex] alert checkfailed: {}", e));
        }
        let _ = crate::modules::tray::update_tray_menu(&app);
    }
    result
...
async fn save_codex_oauth_tokens(tokens: CodexTokens) -> Result<CodexAccount, String> {
 ...
 logger::log_error(&format!("refreshfailed: {}", e));
    }
...
pub async fn codex_oauth_login_start(
    ...
    logger::log_info("Codex OAuth start command triggered");
