...
pub async fn refresh_github_copilot_token(
    ...
    if result.is_ok() {
        if let Err(e) = github_copilot_account::run_quota_alert_if_needed() {
            logger::log_warn(&format!("[QuotaAlert][GitHubCopilot] alert checkfailed: {}", e));
        }
...
pub async fn github_copilot_oauth_login_start() -> Result<GitHubCopilotOAuthStartResponse, String> {
    logger::log_info("GitHub Copilot OAuth start command triggered");
...
pub async fn inject_github_copilot_to_vscode(
 ...
 logger::log_info(&format!("start GitHub Copilot account: {}", account_id));
 ...
 logger::log_warn(&format!(" GitHub Copilot default instanceaccountfailed: {}", e));
    } else {
        logger::log_info(&format!(
        ...
        .await
        {
            ...
            Err(e) => {
                if e.starts_with("APP_PATH_NOT_FOUND:") || e.contains("启动 VS Code 失败") {
                    logger::log_warn(&format!("GitHub Copilot default instancestartfailed: {}", e));
